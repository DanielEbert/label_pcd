...
export class SpatialHash {
	...
	private maxCell: { x: number; y: number; z: number } = {
		x: -Infinity,
		y: -Infinity,
		z: -Infinity
	};
	...
	constructor(
		...
		positions.forEach((position: BABYLON.Vector3, index: number) => {
	...
	private isPointInPolygonXZ(point: BABYLON.Vector3, polygonVertices: BABYLON.Vector3[]): boolean {
		...
		for (let i = 0, j = n - 1; i < n; j = i++) {
			...
			const intersect =
				vi.z > z !== vj.z > z && // One vertex above, one below (or on) the ray's Z
	 ...
	 */
	findParticlesInPolygon(
		polygonVertices: BABYLON.Vector3[],
		particles: BABYLON.CloudPoint[]
	): number[] {
