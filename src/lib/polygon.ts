...
export class Polygon {
	...
	addPoint(position: BABYLON.Vector3) {
		...
		this._rebuildEdges();
		// TODO: doesnt work yet, i don't understand why
	...
	private _rebuildEdges() {
		...
		for (let i = 0; i < numPoints; i++) {
			...
			if (i < numPoints - 1) {
				...
				nextIndex = i + 1;
			} else if (this.options.closePath && numPoints >= 2) {
				// Need >= 2 for closing edge
				// Closing edge last -> 0
	...
	private _updateEdges() {
		...
		if (this.edges.length !== expectedEdgeCount) {
			console.warn(
				`Edge count mismatch (${this.edges.length} vs expected ${expectedEdgeCount}). Rebuilding edges.`
			);
			this._rebuildEdges();
			return;
		}
		...
		for (let i = 0; i < expectedEdgeCount; i++) {
			...
			if (!p1 || !p2) {
				console.warn(
					`Missing point data for edge update at index ${i}. Skipping update for this edge.`
				);
				continue;
			}
			...
			BABYLON.MeshBuilder.CreateTube('tube_update', {
				path: path,
				radius: wallRadius,
	...
	_createNodeMesh(position: BABYLON.Vector3, index: number): BABYLON.Mesh {
		...
		dragBehavior.onDragObservable.add((_) => {
			...
			if (
				...
				pointIndex >= 0 &&
				pointIndex < this.nodePositions.length &&
	...
	_addKeyboardObserver() {
		...
		this._keyboardObserver = this.scene.onKeyboardObservable.add((kbInfo) => {
			...
			if (shiftPressed && (key === 'Delete' || key === 'Backspace')) {
				console.log(
					`Shift+Delete pressed with node ${this._selectedNode.metadata.index} selected. Deleting entire polygon.`
				);
				this.dispose();
			...
			if (!shiftPressed && (kbInfo.event.key === 'Delete' || kbInfo.event.key === 'Backspace')) {
